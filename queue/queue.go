...
// Package queue turingchain底层消息队列模块
package queue
...
type Queue interface {
	...
	Client() Client
	Name() string
	SetConfig(cfg *types.TuringchainConfig)
...
func New(name string) Queue {
	...
	go func() {
		for {
			select {
			case <-q.done:
				qlog.Info("closing turingchain callback")
				return
			case msg := <-q.callback:
				if msg.callback != nil {
					msg.callback(msg)
...
// GetConfig return the queue TuringchainConfig
func (q *queue) GetConfig() *types.TuringchainConfig {
	return q.cfg
}

// Name return the queue name
func (q *queue) SetConfig(cfg *types.TuringchainConfig) {
	if cfg == nil {
...
func (q *queue) Start() {
	...
	select {
	case <-q.done:
		qlog.Info("closing turingchain done")
		//atomic.StoreInt32(&q.isClose, 1)
		break
	case <-q.interrupt:
		qlog.Info("closing turingchain")
		//atomic.StoreInt32(&q.isClose, 1)
		break
	case s := <-c:
