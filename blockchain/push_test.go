...
import (
	...
	bcMocks "github.com/turingchain2020/turingchain/blockchain/mocks"
...
//GetBlockChain :
func (mock *TuringchainMock) GetBlockChain() *BlockChain {
	return mock.chain
}
...
func Test_AddPush_PushNameShouldDiff(t *testing.T) {
	...
	subscribe.URL = "http://localhost:9671"
	err := chain.push.addSubscriber(subscribe)
...
func Test_RecoverPush(t *testing.T) {
	...
	//turingchain的push服务重启后，不会将其添加到task中，
	chainAnother := &BlockChain{
		isRecordBlockSequence: true,
		enablePushSubscribe:   true,
	}
...
//init work
func NewTuringchainMock(cfgpath string, mockapi client.QueueProtocolAPI) *TuringchainMock {
	cfg := types.NewTuringchainConfig(types.GetDefaultCfgstring())
	return newWithConfigNoLock(cfg, mockapi)
}

func NewTuringchainMockWithFlag(cfgpath string, mockapi client.QueueProtocolAPI, isRecordBlockSequence, enablePushSubscribe bool) *TuringchainMock {
	cfg := types.NewTuringchainConfig(types.GetDefaultCfgstring())
...
func createBlocks(t *testing.T, mock33 *TuringchainMock, blockchain *BlockChain, number int64) {
...
func createBlockChain(t *testing.T) (*BlockChain, *TuringchainMock) {
	mock33 := NewTuringchainMock("", nil)
...
func createBlockChainWithFalgSet(t *testing.T, isRecordBlockSequence, enablePushSubscribe bool) (*BlockChain, *TuringchainMock) {
	mock33 := NewTuringchainMockWithFlag("", nil, isRecordBlockSequence, enablePushSubscribe)
