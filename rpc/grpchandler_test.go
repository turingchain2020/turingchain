...
func testVersionOK(t *testing.T) {
	reply := &types.VersionInfo{Turingchain: "6.0.2"}
	qapi.On("Version").Return(reply, nil)
	data, err := g.Version(getOkCtx(), nil)
	assert.Nil(t, err, "the error should be nil")
	assert.Equal(t, "6.0.2", data.Turingchain, "reply should be ok")
}
...
func TestGrpc_GetFork(t *testing.T) {
	types.RegFork("para", func(cfg *types.TuringchainConfig) {
		cfg.SetDappFork("para", "fork100", 100)
	})

	str := types.GetDefaultCfgstring()
	newstr := strings.Replace(str, "Title=\"local\"", "Title=\"turingchain\"", 1)
	cfg := types.NewTuringchainConfig(newstr)
	Init(cfg)
	...
	cfg1 := types.NewTuringchainConfig(types.GetDefaultCfgstring())
	Init(cfg1)
