...
import (
	...
	_ "github.com/turingchain2020/turingchain/rpc/grpcclient" // register grpc multiple resolver
...
func NewGRpcServer(c queue.Client, api client.QueueProtocolAPI) *Grpcserver {
	...
	if rpcCfg.EnableTLS {
		...
		if err != nil {
			panic(fmt.Sprintf("err=%s, if cert.pem not found, run turingchain-cli cert --host=127.0.0.1 to create", err.Error()))
		}
...
func NewJSONRPCServer(c queue.Client, api client.QueueProtocolAPI) *JSONRPCServer {
	...
	j.s = server
	err := server.RegisterName("Turingchain", j.jrpc)
	if err != nil {
		return nil
	}
	return j
...
func New(cfg *types.TuringchainConfig) *RPC {
