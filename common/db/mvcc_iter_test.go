...
func TestGetAllCoinsMVCCIter(t *testing.T) {
	...
	m := getMVCCIter()
	defer m.db.Close()
	kvlist, err := m.AddMVCC(KeyValueList([2]string{"mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp", "1"}, [2]string{"mavl-coins-trc-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTq", "2"}), hashN(0), nil, 0)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)

	kvlist, err = m.AddMVCC(KeyValueList([2]string{"mavl-coins-trc-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp", "3"}, [2]string{"mavl-coins-trc-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTs", "4"}), hashN(1), hashN(0), 1)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)

	kvlist, err = m.AddMVCC(KeyValueList([2]string{"mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp", "5"}, [2]string{"mavl-coins-trc-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTt", "6"}), hashN(2), hashN(1), 2)
	assert.Nil(t, err)
	saveKVList(m.db, kvlist)
	...
	values := listhelper.List([]byte("mavl-coins-trc-"), nil, 100, 1)
	...
	listhelper.IteratorCallback([]byte("mavl-coins-trc-"), nil, 0, 1, func(key, value []byte) bool {
	...
	matchValues = nil
	listhelper.IteratorCallback(([]byte("mavl-coins-trc-")), []byte("mavl-coins-trc-exec-"), 0, 1, func(key, value []byte) bool {
		matchValues = append(matchValues, value)
		return false
	})
	values = matchValues
	...
	matchValues = nil
	listhelper.IteratorCallback(([]byte("mavl-coins-trc-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTq")), []byte("mavl-coins-trc-exec-"), 0, 1, func(key, value []byte) bool {
		matchValues = append(matchValues, value)
		return false
	})
	values = matchValues
...
func TestSimpleMVCCLocalDB(t *testing.T) {
	...
	values, err := kvdb.List([]byte(".-mvcc-.d.mavl-coins-trc-"), nil, 100, 1)
	assert.Nil(t, err)
	...
	v, err := m.GetV([]byte("mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp"), 0)
	assert.Nil(t, err)
	assert.Equal(t, "1", string(v))

	v, err = m.GetV([]byte("mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp"), 1)
	assert.Nil(t, err)
	assert.Equal(t, "1", string(v))

	v, err = m.GetV([]byte("mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp"), 2)
	assert.Nil(t, err)
	assert.Equal(t, "5", string(v))

	v, err = m.GetV([]byte("mavl-coins-trc-exec-16htvcBNSEA7fZhAdLJphDwQRQJaHpyHTp"), 3)
	assert.Nil(t, err)
	assert.Equal(t, "5", string(v))
