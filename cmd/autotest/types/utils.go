...
func SendTxCommand(cmd string) (string, bool) {
	...
	output, err := RunTuringchainCli(strings.Fields(cmd))
	if err != nil {
...
func GetTxInfo(txHash string) (string, bool) {
	...
	bReady := false
	txInfo, err := RunTuringchainCli(strings.Fields(fmt.Sprintf("tx query -s %s", txHash)))
...
func CalcUtxoAvailAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunTuringchainCli(strings.Fields(fmt.Sprintf("privacy showpai -d 1 -a %s", addr)))
...
func CalcUtxoSpendAmount(addr string, txHash string) (float64, error) {
	...
	outStr, err := RunTuringchainCli(strings.Fields(fmt.Sprintf("privacy showpas -a %s", addr)))
