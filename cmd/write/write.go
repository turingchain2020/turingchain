...
import (
	...
	coinstypes "github.com/turingchain2020/turingchain/system/dapp/coins/types"
...
func main() {
	turingchainCfg := types.NewTuringchainConfig(types.GetDefaultCfgstring())
	cfg := initWrite()
...
func scanWrite(cfg *types.TuringchainConfig) {
	for {
		...
		err = rpc.Call("Turingchain.GetTxByAddr", paramsReqAddr, &replyTxInfos)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			continue
		}
		...
		for _, hash := range txHashes {
			...
			err = rpc.Call("Turingchain.QueryTransaction", paramsQuery, &transactionDetail)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue
			}
			...
			var signed string
			rpc.Call("Turingchain.SignRawTx", paramsReqSignRawTx, &signed)
			paramsRaw := rpctypes.RawParm{
				Data: signed,
			}
			var sent string
			rpc.Call("Turingchain.SendTransaction", paramsRaw, &sent)
			f, err := os.OpenFile(heightFile, os.O_RDWR, 0666)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				continue
