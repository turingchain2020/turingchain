...
const monitorTrcLowLimit = 3 * 1e7 * types.Coin
...
func (show *ShowMinerAccount) Get(in *TimeAt, out *interface{}) error {
	...
	log.Info("show 1st balance", "utc", header.BlockTime, "total", totalTrc)

	monitorInterval := calcMoniterInterval(totalTrc)
...
func calcMoniterInterval(totalTrc int64) int64 {
	monitorInterval := int64(baseInterval)
	if totalTrc < monitorTrcLowLimit && totalTrc > 0 {
		monitorInterval = int64(float64(monitorTrcLowLimit) / float64(totalTrc) * float64(baseInterval))
	}
...
func calcIncrease(miner *MinerAccounts, acc1, acc2 []*rpctypes.Account, header *rpctypes.Header) *MinerAccounts {
	...
	for k, v := range miners {
		if v.lastAcc != nil && v.curAcc != nil {
			...
			expectIncrease := float64(miner.Blocks) * float64(trcPreBlock) * (float64(v.curAcc.Frozen) / float64(types.Coin)) / ticketTotal
			...
			expectBlocks := (expectIncrease / trcPreBlock)                               // 一个小时预期挖多少个块
			...
			if err != nil || len(acc) == 0 {
				m.MinerTrcDuring = "0.0000"
			} else {
				minerDelta := total - acc[0].Balance - acc[0].Frozen
				m.MinerTrcDuring = strconv.FormatFloat(float64(minerDelta)/float64(types.Coin), 'f', 4, 64)
			}
