...
import (
	...
	"github.com/turingchain2020/turingchain/cmd/tools/tasks"
)
...
func (up *updateInitStrategy) Run() error {
	mlog.Info("Begin run turingchain update init.go.")
	defer mlog.Info("Run turingchain update init.go finish.")
...
func (up *updateInitStrategy) initMember() error {
	...
	if err != nil || path == "" {
		if len(gopath) > 0 {
			path = filepath.Join(gopath, "/src/github.com/turingchain2020/turingchain/plugin/")
		}
	}
	if packname == "" {
	...
	if len(path) == 0 {
		return errors.New("Turingchain Plugin Not Existed")
	}
