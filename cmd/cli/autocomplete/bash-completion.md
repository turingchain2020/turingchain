# 命令行自动补全
  ...
  目前在ubuntu下用git 等软件你会发现按tab键有提示或补全的功能， 这个功能依赖一个软件包， 叫bash-completion。bash-completion 提供的能力是bash shell 的可编程补全。而turingchain-cli 功能也越来越多，每加一步都看下帮助， 效率不高。 把这个能力加到turingchain-cli里， 应该能提高不少效率。
...
```
# 是不是很简单
$ complete -W "account block trc trade token" turingchain-cli
linj@linj-TM1701:~$ turingchain-cli t
token  trade  
...
```
linj@linj-TM1701:~$ turingchain-cli account 

declare -a COMP_WORDS='([0]="turingchain-cli" [1]="account" [2]="")'
declare -- COMP_CWORD="2"
declare -- COMP_LINE="turingchain-cli account "
...
turingchain-cli 参数补全
```
#!/bin/bash
# 通过turingchain-cli 的help 找到一级的子命令
subcmd_list=("account" "block" "trc" "close" "coins" "config" "evm" "exec" "hashlock" "help" "mempool" "net" "privacy" "relay" "retrieve" "seed" "send" "stat" "ticket" "token" "trade" "tx" "version" "wallet")
#
function _subcmd() {
  local cur
  COMPREPLY=()
...
# 用 _subcmd 补全 turingchain-cli
# _subcmd 通过当前光标所在的输入参数过滤可选的子命令
complete -F _subcmd turingchain-cli
```
...
linj@linj-TM1701:~$ ./turingchain/turingchain-cli t
...
```
linj@linj-TM1701:~$ sudo install subcmd.bash  /usr/share/bash-completion/completions/turingchain-cli
# 重新开个窗口就有用了
linj@linj-TM1701:~$ ./turingchain/turingchain-cli 
account   trc       coins     evm       hashlock  mempool   privacy   retrieve  send      ticket    trade     version   
block     close     config    exec      help      net       relay     seed      stat      token     tx        wallet    
...
### 给turingchain-cli 做个 bash-completion


 地址: https://gitlab.__officeSite__/linj/turingchain-cli-completion
