...
// Package types 实现了turingchain基础结构体、接口、常量等的定义
package types
...
//ExecName  执行器name
func (c *TuringchainConfig) ExecName(name string) string {
...
//GetParaExec  获取平行链执行
func (c *TuringchainConfig) GetParaExec(execer []byte) []byte {
	//必须是平行链
	if !c.IsPara() {
		return execer
	}
...
func (c *TuringchainConfig) ManaeKeyWithHeigh(key string, height int64) string {
