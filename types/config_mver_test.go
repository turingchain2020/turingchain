...
func TestConfigMverInit(t *testing.T) {
	RegFork("store-kvmvccmavl", func(cfg *TuringchainConfig) {
		cfg.RegisterDappFork("store-kvmvccmavl", "ForkKvmvccmavl", MaxHeight)
	})
	cfg := NewTuringchainConfigNoInit(ReadFile("testdata/local.mvertest.toml"))
	cfg.EnableCheckFork(false)
	cfg.turingchainCfgInit(cfg.GetModuleConfig())
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 0), "ticket-turingchaincoin")
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 10), "ticket-turingchaincoinv5")
	assert.Equal(t, cfg.MGStr("mver.consensus.name2", 25), "ticket-turingchaincoinv2")
	...
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", -1), "ticket-turingchaincoin")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 0), "ticket-turingchaincoinv5-enable")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 9), "ticket-turingchaincoinv5-enable")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 10), "ticket-turingchaincoinv5")
	assert.Equal(t, cfg.MGStr("mver.exec.sub.coins.name2", 11), "ticket-turingchaincoinv5")
...
func getP(cfg *TuringchainConfig, height int64) *ChainParam {
	initChainBase()
	initChainTuringchaincoinV3()
...
func initChainTuringchaincoinV3() {
...
func TestInitChainParam(t *testing.T) {
	cfg := NewTuringchainConfig(ReadFile("../cmd/turingchain/turingchain.toml"))
