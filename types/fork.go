...
// SetTestNetFork turingchaincoin test net fork
...
// IsFork 是否系统 fork高度
func (c *TuringchainConfig) IsFork(height int64, fork string) bool {
	return c.forks.IsFork(height, fork)
}

// IsDappFork 是否dapp fork高度
func (c *TuringchainConfig) IsDappFork(height int64, dapp, fork string) bool {
	return c.forks.IsDappFork(height, dapp, fork)
}

// GetDappFork 获取dapp fork高度
func (c *TuringchainConfig) GetDappFork(dapp, fork string) int64 {
	return c.forks.GetDappFork(dapp, fork)
}

// SetDappFork 设置dapp fork高度
func (c *TuringchainConfig) SetDappFork(dapp, fork string, height int64) {
...
func (c *TuringchainConfig) RegisterDappFork(dapp, fork string, height int64) {
	if c.needSetForkZero() {
		height = 0
		if fork == "ForkBlockHash" {
			height = 1
...
// GetFork 获取系统fork高度
func (c *TuringchainConfig) GetFork(fork string) int64 {
	return c.forks.GetFork(fork)
}

// HasFork 是否有系统fork
func (c *TuringchainConfig) HasFork(fork string) bool {
	return c.forks.HasFork(fork)
}

// IsEnableFork 是否使能了fork
func (c *TuringchainConfig) IsEnableFork(height int64, fork string, enable bool) bool {
	if !enable {
		return false
	}
...
func (c *TuringchainConfig) initForkConfig(forks *ForkList) {
	turingchainfork := c.forks.GetAll()
	if turingchainfork == nil {
		panic("turingchain fork not init")
	}
	//开始判断turingchainfork中的system部分是否已经设置
	s := ""
	for k := range turingchainfork {
