...
func TestMergeToml(t *testing.T) {
	newcfg := MergeCfg(ReadFile("../cmd/turingchain/turingchaincoin.toml"), turingchaincoincfg)
	cfg1, err := initCfgString(newcfg)
	assert.Nil(t, err)
	cfg2, err := initCfgString(readFile("testdata/turingchaincoin.toml"))
	assert.Nil(t, err)
	assert.Equal(t, cfg1, cfg2)
}

var turingchaincoincfg = `
TestNet=false
[blockchain]
defCacheSize=128
