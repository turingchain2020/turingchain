...
import (
	...
	"github.com/turingchain2020/turingchain/types/chaincfg"
	tml "github.com/BurntSushi/toml"
)

//Create ...
type Create func(cfg *TuringchainConfig)
...
var (
	...
	cliSysParam   = make(map[string]*TuringchainConfig)                       // map key is title
...
//RegForkInit ...
func RegForkInit(cfg *TuringchainConfig) {
...
//RegExecInit ...
func RegExecInit(cfg *TuringchainConfig) {
	runonce.Do(func() {
...
//NewTuringchainConfig ...
func NewTuringchainConfig(cfgstring string) *TuringchainConfig {
	turingchainCfg := NewTuringchainConfigNoInit(cfgstring)
	turingchainCfg.turingchainCfgInit(turingchainCfg.mcfg)
	return turingchainCfg
}

//NewTuringchainConfigNoInit ...
func NewTuringchainConfigNoInit(cfgstring string) *TuringchainConfig {
	cfg, sub := InitCfgString(cfgstring)
	turingchainCfg := &TuringchainConfig{
		mcfg:        cfg,
		scfg:        sub,
	...
	// 先将每个模块的fork初始化到TuringchainConfig中，然后如果需要再将toml中的替换
	turingchainCfg.setDefaultConfig()
	turingchainCfg.setFlatConfig(cfgstring)
	turingchainCfg.setMver(cfgstring)
	// TODO 需要测试是否与NewTuringchainConfig分开
	RegForkInit(turingchainCfg)
	RegExecInit(turingchainCfg)
...
//GetModuleConfig ...
func (c *TuringchainConfig) GetModuleConfig() *Config {
	return c.mcfg
}

//GetSubConfig ...
func (c *TuringchainConfig) GetSubConfig() *ConfigSubModule {
	return c.scfg
}

//EnableCheckFork ...
func (c *TuringchainConfig) EnableCheckFork(enable bool) {
	c.enableCheckFork = false
}

//GetForks ...
func (c *TuringchainConfig) GetForks() (map[string]int64, error) {
	if c.forks == nil {
...
func (c *TuringchainConfig) setDefaultConfig() {
	c.S("TestNet", false)
...
func (c *TuringchainConfig) setFlatConfig(cfgstring string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TuringchainConfig) setChainConfig(key string, value interface{}) {
	c.chainConfig[key] = value
}

func (c *TuringchainConfig) getChainConfig(key string) (value interface{}, err error) {
...
// Init 初始化
func (c *TuringchainConfig) turingchainCfgInit(cfg *Config) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TuringchainConfig) needSetForkZero() bool {
	if c.isLocal() {
		return true
	} else if c.isPara() &&
...
func (c *TuringchainConfig) setTestNet(isTestNet bool) {
	if !isTestNet {
...
// GetP 获取ChainParam
func (c *TuringchainConfig) GetP(height int64) *ChainParam {
...
func (c *TuringchainConfig) GetMinerExecs() []string {
	return c.minerExecs
}

func (c *TuringchainConfig) setMinerExecs(execs []string) {
	if len(execs) > 0 {
		c.minerExecs = execs
	}
}
...
func (c *TuringchainConfig) GetFundAddr() string {
	return c.MGStr("mver.consensus.fundKeyAddr", 0)
}

// G 获取ChainConfig中的配置
func (c *TuringchainConfig) G(key string) (value interface{}, err error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TuringchainConfig) MG(key string, height int64) (value interface{}, err error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.mver == nil {
		panic("mver is nil")
...
func (c *TuringchainConfig) GStr(name string) string {
	value, err := c.G(name)
	if err != nil {
		return ""
	}
...
func (c *TuringchainConfig) MGStr(name string, height int64) string {
...
func (c *TuringchainConfig) GInt(name string) int64 {
	value, err := c.G(name)
	if err != nil {
		return 0
	}
...
func (c *TuringchainConfig) MGInt(name string, height int64) int64 {
	value, err := c.MG(name, height)
	if err != nil {
		return 0
	}
...
func (c *TuringchainConfig) IsEnable(name string) bool {
...
func (c *TuringchainConfig) MIsEnable(name string, height int64) bool {
...
func (c *TuringchainConfig) HasConf(key string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// S 设置chainConfig配置
func (c *TuringchainConfig) S(key string, value interface{}) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *TuringchainConfig) SetTitleOnlyForTest(ti string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.title = ti
...
// GetTitle 获取title
func (c *TuringchainConfig) GetTitle() string {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.title
}
...
func (c *TuringchainConfig) GetCoinSymbol() string {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.coinSymbol
}

func (c *TuringchainConfig) isLocal() bool {
...
func (c *TuringchainConfig) GetMinTxFeeRate() int64 {
	return c.GInt("MinTxFeeRate")
}

// GetMaxTxFeeRate get max transaction fee rate
func (c *TuringchainConfig) GetMaxTxFeeRate() int64 {
	return c.GInt("MaxTxFeeRate")
}

// GetMaxTxFee get max transaction fee
func (c *TuringchainConfig) GetMaxTxFee() int64 {
	return c.GInt("MaxTxFee")
}

// SetTxFeeConfig 设置交易费相关配置
func (c *TuringchainConfig) SetTxFeeConfig(minTxFeeRate, maxTxFeeRate, maxTxFee int64) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// SetMinFee 设置最小费用
func (c *TuringchainConfig) SetMinFee(fee int64) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.setTxFeeConfig(fee, fee*100, fee*10000)
}

func (c *TuringchainConfig) isPara() bool {
	return strings.Count(c.title, ".") == 3 && strings.HasPrefix(c.title, ParaKeyX)
}

func (c *TuringchainConfig) isTestPara() bool {
...
func (c *TuringchainConfig) IsMyParaExecName(exec string) bool {
...
// IsTestNet 是否测试链
func (c *TuringchainConfig) IsTestNet() bool {
	return c.IsEnable("TestNet")
}

// GetParaName 获取平行链name
func (c *TuringchainConfig) GetParaName() string {
	if c.IsPara() {
		return c.GetTitle()
	}
	return ""
...
func (c *TuringchainConfig) setMver(cfgstring string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
type ConfQuery struct {
	cfg    *TuringchainConfig
	prefix string
}

// Conf 配置
func Conf(cfg *TuringchainConfig, prefix string) *ConfQuery {
...
// ConfSub 子模块配置
func ConfSub(cfg *TuringchainConfig, name string) *ConfQuery {
...
//SetCliSysParam ...
func SetCliSysParam(title string, cfg *TuringchainConfig) {
	if cfg == nil {
		panic("set cli system TuringchainConfig param is nil")
	}
	cliSysParam[title] = cfg
}

//GetCliSysParam ...
func GetCliSysParam(title string) *TuringchainConfig {
...
func AssertConfig(check interface{}) {
	if check == nil {
		panic("check object is nil (TuringchainConfig)")
	}
}

// GetChainID 获取链ID,提供给其他模块使用
func (c *TuringchainConfig) GetChainID() int32 {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.chainID
}
