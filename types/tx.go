...
func (txgroup *Transactions) SetExpire(cfg *TuringchainConfig, n int, expire time.Duration) {
	if n >= len(txgroup.GetTxs()) {
		return
	}
...
//IsExpire 交易是否过期
func (txgroup *Transactions) IsExpire(cfg *TuringchainConfig, height, blocktime int64) bool {
	txs := txgroup.Txs
	for i := 0; i < len(txs); i++ {
...
func (txgroup *Transactions) CheckWithFork(cfg *TuringchainConfig, checkFork, paraFork bool, height, minfee, maxFee int64) error {
	txs := txgroup.Txs
	if len(txs) < 2 {
...
func (txgroup *Transactions) Check(cfg *TuringchainConfig, height, minfee, maxFee int64) error {
...
func (tx *TransactionCache) Check(cfg *TuringchainConfig, height, minfee, maxFee int64) error {
	if !tx.checked {
		tx.checked = true
...
//Check 交易检测
func (tx *Transaction) Check(cfg *TuringchainConfig, height, minfee, maxFee int64) error {
	group, err := tx.GetTxGroup()
	if err != nil {
		return err
	}
...
//SetExpire 设置交易过期时间
func (tx *Transaction) SetExpire(cfg *TuringchainConfig, expire time.Duration) {
	//Txheight处理
...
//IsExpire 交易是否过期
func (tx *Transaction) IsExpire(cfg *TuringchainConfig, height, blocktime int64) bool {
...
//GetTxHeight 获取交易高度
func GetTxHeight(cfg *TuringchainConfig, valid int64, height int64) int64 {
	if cfg.IsPara() {
		return -1
	}
