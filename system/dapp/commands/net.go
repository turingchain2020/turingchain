...
func peerInfo(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.GetPeerInfo", req, &res)
	ctx.Run()
}
...
func isClockSync(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res bool
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.IsNtpClockSync", nil, &res)
	ctx.Run()
}
...
func isSync(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res bool
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.IsSync", nil, &res)
	ctx.Run()
}
...
func netInfo(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.GetNetInfo", req, &res)
	ctx.Run()
}
...
func netProtocols(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.NetProtocols", &types.ReqNil{}, &res)
	ctx.Run()
}
...
func fatalFailure(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res int64
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.GetFatalFailure", nil, &res)
	ctx.Run()
}
...
func timestatus(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Turingchain.GetTimeStatus", nil, &res)
	ctx.Run()
}
