...
func getTotalCoinsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "total_coins",
		Short: "Get total amount of a token (default: trc of current height)",
		Run:   totalCoins,
	}
...
func addTotalCoinsCmdFlags(cmd *cobra.Command) {
	cmd.Flags().StringP("symbol", "s", "trc", "token symbol")
...
func totalCoins(cmd *cobra.Command, args []string) {
	...
	if actual != "" {
		...
		for count = 100; count == 100; {
			...
			err = rpc.Call("Turingchain.GetTotalCoins", params, &res)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				return
			}
...
func getExecBalanceCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "exec_balance",
		Short: "Get the exec amount of a token of one address (default: all exec-addr trc of current height of one addr)",
		Run:   execBalance,
	}
...
func execBalance(cmd *cobra.Command, args []string) {
	...
	for {
		...
		var str string
		err = rpc.Call("Turingchain.GetExecBalance", reqParam, &str)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
...
func getLastBlock(rpc *jsonclient.JSONClient) (*rpctypes.Header, error) {
	...
	res := &rpctypes.Header{}
	err := rpc.Call("Turingchain.GetLastHeader", nil, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlockHash(height int64, rpc *jsonclient.JSONClient) (string, error) {
	...
	err := rpc.Call("Turingchain.GetBlockHash", params, &res)
	if err != nil {
		return "", err
	}
	return res.Hash, nil
...
func queryTotalFeeWithHash(blockHash string, rpc *jsonclient.JSONClient) (*types.TotalFee, error) {
	...
	res := &types.TotalFee{}
	err = rpc.Call("Turingchain.QueryTotalFee", params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlocks(start, end int64, rpc *jsonclient.JSONClient) (*rpctypes.BlockDetails, error) {
	...
	err := rpc.Call("Turingchain.GetBlocks", params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
