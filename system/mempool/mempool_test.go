...
import (
	...
	_ "github.com/turingchain2020/turingchain/system/consensus/init"
...
func initEnv3() (queue.Queue, queue.Module, queue.Module, *Mempool) {
	cfg := types.NewTuringchainConfig(types.ReadFile("../../cmd/turingchain/turingchain.test.toml"))
...
func initEnv4(size int) (queue.Queue, *Mempool) {
	...
	cfg := types.NewTuringchainConfig(types.ReadFile("testdata/turingchain.test.toml"))
	mcfg := cfg.GetModuleConfig()
	var q = queue.New("channel")
	q.SetConfig(cfg)
	blockchainProcess(q)
...
func createTx(cfg *types.TuringchainConfig, priv crypto.PrivKey, to string, amount int64) *types.Transaction {
	v := &cty.CoinsAction_Transfer{Transfer: &types.AssetsTransfer{Amount: amount}}
	transfer := &cty.CoinsAction{Value: v, Ty: cty.CoinsActionTransfer}
