...
import (
	...
	log "github.com/turingchain2020/turingchain/common/log"
...
func BenchmarkSendTx(b *testing.B) {
	...
	assert.Nil(b, err)
	subcfg.Consensus["solo"] = solocfg
	cfg.GetModuleConfig().RPC.JrpcBindAddr = "localhost:9671"
	cfg.GetModuleConfig().RPC.GrpcBindAddr = "localhost:9672"
	...
	b.Run("SendTx-GRPC", func(b *testing.B) {
		gcli, _ := grpcclient.NewMainChainClient(cfg, "localhost:9672")
		b.RunParallel(func(pb *testing.PB) {
			for pb.Next() {
				tx := util.CreateNoneTxWithTxHeight(cfg, priv, types.LowAllowPackHeight/2)
	...
	b.Run("SendTx-JSONRPC", func(b *testing.B) {
		b.RunParallel(func(pb *testing.PB) {
			for pb.Next() {
				...
				resp, _ := http.Post("http://localhost:9671", "application/json", bytes.NewBufferString(poststr))
...
func BenchmarkSoloNewBlock(b *testing.B) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(addr string) {
			start <- struct{}{}
			conn, err := grpc.Dial("localhost:9672", grpc.WithInsecure())
			if err != nil {
				panic(err.Error())
			}
			defer conn.Close()
