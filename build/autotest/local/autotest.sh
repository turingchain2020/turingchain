...
turingchainConfig="turingchain.test.toml"
turingchainBlockTime=1
autoTestCheckTimeout=10

function config_turingchain() {

    # shellcheck disable=SC2015
    echo "# config turingchain solo test"
    # update test environment
    sed -i $sedfix 's/^Title.*/Title="local"/g' ${turingchainConfig}
    # grep -q '^TestNet' ${turingchainConfig} && sed -i $sedfix 's/^TestNet.*/TestNet=true/' ${turingchainConfig} || sed -i '/^Title/a TestNet=true' ${turingchainConfig}

    if grep -q '^TestNet' ${turingchainConfig}; then
        sed -i $sedfix 's/^TestNet.*/TestNet=true/' ${turingchainConfig}
    else
        sed -i $sedfix '/^Title/a TestNet=true' ${turingchainConfig}
    fi

    #update fee
    #    sed -i $sedfix 's/Fee=.*/Fee=100000/' ${turingchainConfig}

    #update wallet store driver
    #    sed -i $sedfix '/^\[wallet\]/,/^\[wallet./ s/^driver.*/driver="leveldb"/' ${turingchainConfig}
}
...
function start_turingchain() {

    echo "# start solo turingchain"
    rm -rf ../local/datadir ../local/logs ../local/grpc33.log
    ./turingchain -f turingchain.test.toml >/dev/null 2>&1 &
    local SLEEP=1
    sleep ${SLEEP}
...
function stop_turingchain() {

    rv=$?
    echo "=========== #stop turingchain ============="
    ${CLI} close || true
    #wait close
    sleep ${turingchainBlockTime}
...
trap "stop_turingchain" INT TERM EXIT
