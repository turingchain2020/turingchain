...
func isAllowKeyWrite(e *executor, key, realExecer []byte, tx *types.Transaction, index int) bool {
	...
	// 历史原因做只针对对turingchaincoin的fork特殊化处理一下
...
func isAllowLocalKey(cfg *types.TuringchainConfig, execer []byte, key []byte) error {
...
func isAllowLocalKey2(cfg *types.TuringchainConfig, execer []byte, key []byte) error {
	if len(execer) < 1 {
