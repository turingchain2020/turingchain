...
func TestKeyAllow(t *testing.T) {
	...
	execInit(nil)
	key := []byte("mavl-coins-trc-exec-1wvmD6RNHzwhY4eN75WnM6JcaAvNQ4nHx:19xXg1WHzti5hzBRTUphkM8YmuX6jJkoAA")
	exec := []byte("retrieve")
...
func TestKeyAllow_evm(t *testing.T) {
	exect, _ := initEnv(types.GetDefaultCfgstring())
	execInit(nil)
	key := []byte("mavl-coins-trc-exec-1GacM93StrZveMrPjXDoz5TxajKa9LM5HG:19EJVYexvSn1kZ6MWiKcW14daXsPpdVDuF")
...
func Register(cfg *types.TuringchainConfig) {
	testRegOnce.Do(func() {
...
func TestCheckTx(t *testing.T) {
	exec, q := initEnv(types.ReadFile("../cmd/turingchain/turingchain.test.toml"))
	cfg := exec.client.GetConfig()
