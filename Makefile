...
SRC := github.com/turingchain2020/turingchain/cmd/turingchain
SRC_CLI := github.com/turingchain2020/turingchain/cmd/cli
SRC_SIGNATORY := github.com/turingchain2020/turingchain/cmd/signatory-server
SRC_MINER := github.com/turingchain2020/turingchain/cmd/miner_accounts
...
SRC_AUTOTEST := github.com/turingchain2020/turingchain/cmd/autotest
LDFLAGS := -ldflags "-w -s"
...
all: ## Builds for multiple platforms
	@gox  $(LDFLAGS) $(SRC)
	@cp cmd/turingchain/turingchain.toml build/
	@cp cmd/turingchain/turingchaincoin.toml build/
	@mv turingchain* build/
...
release: ## Build the binary file
	...
	@cp cmd/turingchain/turingchain.toml build/
	@cp cmd/turingchain/turingchaincoin.toml build/
	@cp cmd/turingchain/turingchain.para.toml build/

cli: ## Build cli binary
	@go build -v -i -o $(CLI) $(SRC_CLI)

execblock: ## Build cli binary
	@go build -v -i -o build/execblock github.com/turingchain2020/turingchain/cmd/execblock
...
docker-compose: ## build docker-compose for turingchain run
	@cd build && if ! [ -d ci ]; then \
	 ...
	 cp turingchain* Dockerfile  docker-compose* system-test* ci/ && cd ci/ && ./docker-compose-pre.sh run $(PROJ) $(DAPP)  && cd ../..
...
mock:
	...
	@cd types/ && mockery -name=TuringchainClient && mv mocks/TuringchainClient.go mocks/turingchainclient.go && cd -
...
addupstream:
	git remote add upstream https://github.com/turingchain2020/turingchain.git
	git remote -v
...
pull:
	@remotelist=$$(git remote | grep ${name});if [ -z $$remotelist ]; then \
		echo ${remotelist}; \
		git remote add ${name} https://github.com/${name}/turingchain.git ; \
	fi;
	git fetch ${name}
	git checkout ${name}/${b}
