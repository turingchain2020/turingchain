...
// Package account 实现turingchain 区块链资产操作
package account
...
func NewCoinsAccount(cfg *types.TuringchainConfig) *DB {
	prefix := "mavl-coins-" + cfg.GetCoinSymbol() + "-"
	return newAccountDB(cfg, prefix)
}

// NewAccountDB 新建DB账户
func NewAccountDB(cfg *types.TuringchainConfig, execer string, symbol string, db dbm.KV) (*DB, error) {
...
func newAccountDB(cfg *types.TuringchainConfig, prefix string) *DB {
	acc := &DB{cfg: cfg}
