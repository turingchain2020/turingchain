pipeline {
    ...
    environment {
        GOPATH = "${WORKSPACE}"
        PROJ_DIR = "${WORKSPACE}/src/gitlab.__officeSite__/turingchain/turingchain"
    }
    ...
    options {
        ...
        checkoutToSubdirectory "src/gitlab.__officeSite__/turingchain/turingchain"
    }
    ...
    stages {
        ...
        stage('test'){
            agent {
                docker{
                    image 'suyanlong/turingchain-run:latest'
                }
            }
            ...
            steps {
                dir("${PROJ_DIR}"){
                    gitlabCommitStatus(name: 'deploy'){
                        ...
                        sh "cd build && mkdir ${env.BUILD_NUMBER} && cp ci/* ${env.BUILD_NUMBER} -r && cp turingchain* Dockerfile* docker* *.sh ${env.BUILD_NUMBER}/ && cd ${env.BUILD_NUMBER}/ && ./docker-compose-pre.sh run ${env.BUILD_NUMBER} all "
                    }
                }
            }
